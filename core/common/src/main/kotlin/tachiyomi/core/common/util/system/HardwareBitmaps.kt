...
internal object HardwareBitmaps {
    ...
    val HARDWARE_BITMAP_UNSUPPORTED = when (Build.VERSION.SDK_INT) {
        Build.VERSION_CODES.O -> {
            ...
            model != null &&
                (model.removePrefix("SAMSUNG-").startsWith("SM-") || Build.DEVICE in OREO_BROKEN_DEVICES)
        }
        Build.VERSION_CODES.O_MR1 -> {
            Build.DEVICE in OREO_MR1_BROKEN_DEVICES
        }
        else -> {
            false
        }
...
// Samsung Galaxy (ALL) is matched by model prefix in addition to this list.
private val OREO_BROKEN_DEVICES = arrayOf(
...
private val OREO_MR1_BROKEN_DEVICES = arrayOf(
