...
internal class ReaderViewModelInitTest {
    ...
    private fun chaptersOf(vm: ReaderViewModel, chapter: ReaderChapter) {
    ...
    @Test
    fun mergedSourceReadsReferences() {
        runBlocking {
    ...
    fun metadataSourceRaisesMeta() {
        runBlocking {
            ...
            harness.uiPreferences.relativeTime.set(false)
            vm.publishInitialState(harness.manga, source, none)
            vm.state.value.meta.shouldBeNull()
            vm.state.value.dateRelativeTime shouldBe false
