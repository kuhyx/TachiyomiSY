...
internal class LibrarySettingsHarness(trackerCount: Int) {
    ...
    private val trackers: List<Tracker> = List(trackerCount) { DummyTracker(id = it + 1L, name = "Tracker ${it + 1}") }
    private val setSort = mockk<SetSortModeForCategory>().also {
        coEvery { it.await(any<tachiyomi.domain.category.model.Category?>(), any(), any()) } answers {
            sorts += "${secondArg<Any>()} ${thirdArg<Any>()}"
        }
    }
