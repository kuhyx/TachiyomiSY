...
internal class Anilist(id: Long) : BaseTracker(id, "AniList"), DeletableTracker {
    ...
    internal val scorePreference = trackPreferences.anilistScoreType
    ...
    override fun getScoreList(): List<String> = scoreList()
    ...
    override fun indexToScore(index: Int): Double = scoreForIndex(index)

    override fun displayScore(track: DomainTrack): String = displayedScore(track)
    ...
    override suspend fun delete(track: DomainTrack) {
        if ((track.libraryId ?: 0L) == 0L) {
            val libManga = api.findLibManga(track.toDbTrack(), getUsername().toInt()) ?: return
            return api.deleteLibManga(track.copy(libraryId = libManga.libraryId))
        }
